...
class ProductController extends Controller
{
    ...
    public function store(StoreProductRequest $request)
    {
        ...
        foreach ($request->input('additional_images', []) as $file) {
            $product->addMedia(storage_path('tmp/uploads/' . $file))->toMediaCollection('additional_images');
        }
    ...
    public function update(UpdateProductRequest $request, Product $product)
    {
        ...
        if (count($product->additional_images) > 0) {
            foreach ($product->additional_images as $media) {
        ...
        $media = $product->additional_images->pluck('file_name')->toArray();
